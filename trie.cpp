...
struct trie
{
  struct node
  {
      ...
      if(queryString[0]!='_')
	{
	  ...
	  if((curAnsString[0]==NullIterator) &&
	     (iter==child.end()))
	    return false;
	  locally_changed = (curAnsString[0]==NullIterator);
	  if(iter->second->next_match(&(queryString[1]),
				      &(curAnsString[1]),
				      changed || locally_changed))
	    {
	      curAnsString[0]=child.find(queryString[0]);
	      return true;
	    }
	  else
	    {
	      curAnsString[0]=NullIterator;
	      return false;
	    }
	}
  ...
  static const std::map<char,node*>::iterator NullIterator;
  node* root;
...
const std::map<char,trie::node*>::iterator trie::NullIterator;
struct trieQuery
{
  trie* t;
  char* queryString;
  char* ansString;
  std::vector<std::map<char,trie::node*>::iterator> ansIteratorString;
  trieQuery(trie*& _t,const char* _qString):t(_t)
  {
    ...
    ansIteratorString.resize(queryLength+1);
  ...
  {
    aString = ansString;
    bool newmatch = t->next_match(queryString,&(ansIteratorString[0]));
    if(newmatch)
      {
	int n = ansIteratorString.size()-1;
	for(int i=0;i<n;++i)
	  ansString[i]=ansIteratorString[i]->first;
      }
    return newmatch;
  }  
  ~trieQuery()
